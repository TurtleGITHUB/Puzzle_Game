...
function init() {
   ...
   // Loop through the puzzle buttons objects attaching the onclick event handler to each one
   for(var i = 0; i < puzzleButtons.length; i++) {
      puzzleButtons[i].onclick = swapPuzzle;
   }
   setupPuzzle();

   // Add an event listener for the mouseuevent
   document.addEventListener("mouseup", endBackground);

   // Add an event listener to the show solution button
   document.getElementById("solve").addEventListener("click",
      function() {
         // Remove the inline backgroundColor style form each cell
...
function swapPuzzle(e) {
   ...
   if(confirm("You will lose all of your work on the puzzle! Continue?")){
      // figure out which button generated the click event by its ID
      ...
      // switch statement to decide which puzzle to load based on the pussleID value
      switch (puzzleID) {
...
function setupPuzzle() {
   ...
   // Creat arrays of the filled and empty cells
...
function endBackground() {
   ...
   for(var i = 0; i < puzzleCells.length; i++) {
      ...
      puzzleCells[i].style.cursor = "url(jpf_pencil), pointer";
   }
